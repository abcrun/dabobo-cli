...
const envs = require(path.resolve('./.env.json'));
...
module.exports = (mode, env) => {
  ...
  const config = {
    entry: /[tj]sx/i.test(entry) ? ['react-hot-loader/patch', entry] : [entry],
    output: {
      filename: output.js,
  ...
  plugins.push(
    new MiniCssExtractPlugin({
      filename: mode === 'development' ? '[name].css' : output.css,
      chunkFilename: mode === 'development' ? '[name].css' : output.chunkcss,
    }),
