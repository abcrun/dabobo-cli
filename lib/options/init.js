...
module.exports = () => {
  inquirer.prompt(questions).then((answers) => {
    console.log(
      chalk.green('Creating required files and having a good journey!')
    );

    const { commitlint, language, frame } = answers;
    let entry;

    if (commitlint) files.push('commitlint.config.js');
    if (language) {
      files.push('tsconfig.json');
      files.push('assets.d.ts');
      entry = !frame ? './src/index.ts' : './src/index.tsx';
      if (frame) files.push('./src/App.tsx');
    } else {
      entry = !frame ? './src/index.js' : './src/index.jsx';
      if (frame) files.push('./src/App.jsx');
    }
    files.push(entry);
    ...
    fs.mkdir(resolve('./src'), () => {});
    fs.mkdir(resolve('./mock'), () => {});
    fs.mkdir(resolve('./public'), () => {});

    fs.copyFile(
      resolve(__dirname, '../../templates/mocker.js'),
      resolve('./mock', 'index.js'),
      (err) => {
        if (err) console.error(chalk.red(JSON.stringify(err)));
        else console.log(chalk.cyan('    create mock/index.js'));
      }
    );
    ...
    fs.writeFile(
      ...
      (err) => {
        if (err) console.error(chalk.red(JSON.stringify(err)));
        else console.log(chalk.cyan('    create .init'));
      }
    );
  });
};
